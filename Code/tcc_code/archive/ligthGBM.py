# Código completo: Federated Learning com LightGBM e Flower (Bagging + Cyclic)
...
def transform_dataset_to_lgb_dataset(hf_dataset):
...
# CLASSE DO CLIENTE LIGHTGBM
class LightGBMClient(Client):
    def __init__(self, train_data, valid_data, num_train, num_val, num_local_round, params, train_method, cid: int, X_valid=None, y_valid=None):
...
def client_fn(context: Context):
    ...
    return LightGBMClient(train_data, valid_data, num_train, num_val, num_local_round, params, train_method, partition_id, X_valid=valid_X, y_valid=valid_y).to_client()
...
class FedLightGBMBagging(Strategy):
    """Estratégia de Bagging para LightGBM - agrega modelos"""
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        # Para bagging, agregamos o primeiro modelo (simplificação)
...
class FedLightGBMCyclic(Strategy):
    """Estratégia Cíclica para LightGBM - treina sequencialmente"""
        ...
        if server_round == 0:
            ...
            try:
                ...
                if parameters.tensors:
                    ...
                    return float(1.0 - accuracy), {"accuracy": float(accuracy), "auc": 0.5}
...
def server_fn(context: Context):
    ...
    if centralised_eval:
        ...
        test_lgb_dataset = transform_dataset_to_lgb_dataset(test_set)
    ...
    evaluate_fn = get_evaluate_fn(test_lgb_dataset, params, X_test=X_test_server if centralised_eval else None, y_test=y_test_server if centralised_eval else None) if centralised_eval else None
...
log(INFO, "GPU disponível (torch): %s | usando device: %s", USE_GPU, device_type)
...
# EXECUTAR SIMULAÇÕES
print(f"\n{'='*80}")
print("INICIANDO EXPERIMENTOS DE FEDERATED LEARNING COM LIGHTGBM")
print(f"{'='*80}")
...
save_metrics_to_file(all_metrics, f"federated_lightgbm_metrics_{int(time.time())}.json")

print(f"\n{'='*80}")
print("EXPERIMENTOS CONCLUÍDOS")
print(f"{'='*80}")

log(INFO, "Simulações finalizadas com LightGBM (Bagging + Cyclic) e métricas avançadas.")