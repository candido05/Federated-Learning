#!/usr/bin/env python3
"""
Script para executar todos os experimentos do XGBoost
...
from algorithms import run_xgboost_experiment


def main():
    """Executa todos os experimentos do XGBoost"""
    ...
    print("="*80)
    print("EXECUÇÃO COMPLETA DE EXPERIMENTOS XGBOOST")
    print("="*80)
    print(f"\nConfiguração:")
    ...
    for strategy in strategies:
        print(f"\n{'#'*80}")
        print(f"# INICIANDO: XGBoost - {strategy.upper()}")
        print(f"{'#'*80}\n")
        ...
        try:
            ...
            if result is not None:
                results[f"xgboost_{strategy}"] = result
                print(f"\n✅ Experimento XGBoost-{strategy} CONCLUÍDO COM SUCESSO!")
            else:
                print(f"\n❌ Experimento XGBoost-{strategy} falhou (resultado None)")

        except Exception as e:
            print(f"\n❌ ERRO no experimento XGBoost-{strategy}: {e}")
            import traceback
            traceback.print_exc()
    ...
    if results:
        print("\n📁 Logs salvos em:")
        print("   logs/xgboost/YYYYMMDD_HHMMSS_cyclic/")
        print("   logs/xgboost/YYYYMMDD_HHMMSS_bagging/")
