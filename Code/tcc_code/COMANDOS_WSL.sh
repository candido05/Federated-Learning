...
if [ -d "venv" ]; then
    ...
    echo -e "${YELLOW}⚠ Ambiente virtual não encontrado em venv/${NC}"
    echo "Ajuste o caminho do ambiente virtual"
fi
echo ""

echo -e "${BLUE}3. VERIFICANDO DEPENDÊNCIAS${NC}"
python -c "import flwr, xgboost, lightgbm, catboost; print('✓ Todas as dependências instaladas')" 2>/dev/null && echo -e "${GREEN}✓ OK${NC}" || {
...
case $choice in
    ...
    3)
        ...
        ;;
    *)
        echo -e "${GREEN}Ambiente preparado! Use os comandos acima quando quiser executar.${NC}"
        ;;
esac
