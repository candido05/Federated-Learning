#!/usr/bin/env python3
"""
Script para executar todos os experimentos do LightGBM
...
from algorithms import run_lightgbm_experiment


def main():
    """Executa todos os experimentos do LightGBM"""
    ...
    print("="*80)
    print("EXECUÇÃO COMPLETA DE EXPERIMENTOS LIGHTGBM")
    print("="*80)
    print(f"\nConfiguração:")
    ...
    for strategy in strategies:
        print(f"\n{'#'*80}")
        print(f"# INICIANDO: LightGBM - {strategy.upper()}")
        print(f"{'#'*80}\n")
        ...
        try:
            ...
            if result is not None:
                results[f"lightgbm_{strategy}"] = result
                print(f"\n✅ Experimento LightGBM-{strategy} CONCLUÍDO COM SUCESSO!")
            else:
                print(f"\n❌ Experimento LightGBM-{strategy} falhou (resultado None)")

        except Exception as e:
            print(f"\n❌ ERRO no experimento LightGBM-{strategy}: {e}")
            import traceback
            traceback.print_exc()
    ...
    if results:
        print("\n📁 Logs salvos em:")
        print("   logs/lightgbm/YYYYMMDD_HHMMSS_cyclic/")
        print("   logs/lightgbm/YYYYMMDD_HHMMSS_bagging/")
